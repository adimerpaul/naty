...
`;f.forEach(M=>{const p=l(M.producto?.nombre??M.nombre??M.descripcion??""),D=Number(M.cantidad??0),k=Number(M.precio??0),w=Number(M.subTotal??D*k),B=l(M.producto_id??M.product_id??M.producto?.id??"");g+=`<div style='font-size: 12px'><b>${B?B+" - ":""}${p}</b></div>`,g+=`
      <div>
